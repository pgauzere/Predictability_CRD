...
PaleoClimSpecies_site_time <- join(paleoOcc, sitesClim)
...
write.csv(PaleoClimSpecies_site_time_SAMPLED, file="results/niches_paleo_all.csv")


Paleo_niches_summary <- ddply(na.omit(PaleoClimSpecies_site_time), .(taxon), summarise, 
...
PaleoClimSpecies_site_time$species <- as.factor(PaleoClimSpecies_site_time$taxon)
...
PaleoClimSpecies_site_time_SAMPLED <- ddply(PaleoClimSpecies_site_time, .(taxon, time),  function(x) {
...
Paleo_niches_summary_SAMPLED<-ddply(PaleoClimSpecies_site_time_SAMPLED, .(taxon), summarise, 
                                    mean.Monthly.maxTemp = mean(ClimTmax.ave),
                                    sd.Monthly.maxTemp = sd(ClimTmax.ave),
                                    mean.Monthly.Prec = mean(ClimPrec.ave),
                                    sd.Monthly.Prec = sd(ClimPrec.ave), .progress="text")
...
write.csv(Paleo_niches_summary_SAMPLED, file="results/niches_paleo_summary.csv")
