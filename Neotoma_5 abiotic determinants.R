...
predictors$humanPresence <- as.factor(predictors$humanPresence)
...
s_ice <- readShapeSpatial('data/basemall/basemall/21calslcticesea.shp', proj4string=CRS("+proj=longlat +datum=WGS84 +units=m +ellps=WGS84 +towgs84=0,0,0"))
s_ice <- s_ice["A1CALSEA_I"]
s_ice <- s_ice[s_ice $A1CALSEA_I == -1,]

load('results/predictors.Rdata')
predictors$was_glaciated <- !is.na(raster::extract(s_ice, predictors[,c("Longitude","Latitude")])[,"A1CALSEA_I"])

points_sites <- predictors
coordinates(points_sites) <- ~ Longitude + Latitude
proj4string(points_sites) <- proj4string(s_ice)

library(geosphere)
test <- dist2Line(points_sites, s_ice)
predictors$dist_refugia <- test[,1]
predictors$dist_refugia[predictors$was_glaciated==F] <-  0
hist(predictors$dist_refugia)
hist(sqrt(predictors$dist_refugia))

fort_s_ice <- fortify(s_ice)

worldmap +
  geom_polygon(data=fort_s_ice, aes(long, lat, group=group), alpha=0.05, fill='blue', col="blue")+
  geom_point(data=predictors, aes(Longitude, Latitude, fill=dist_to_LGM),size=3 , shape=21)+
...
hist_distRefugia<-
  ggplot(predictors, aes(x=dist_refugia))+
  geom_histogram(fill="blue", bins=100)+
  # scale_x_continuous(trans="log1p")+
  theme_classic()
...
distRefugia <-
  worldmap + 
  geom_point(data=predictors, aes(Longitude, Latitude, fill=dist_refugia/100000),size=3 , shape=21)+
  scale_fill_gradientn(colours=cubeHelix(100))+
  theme_classic()+
  theme(legend.position="bottom")
