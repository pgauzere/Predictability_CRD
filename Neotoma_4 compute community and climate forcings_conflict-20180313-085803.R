...
load("results/niches_all.Rdata")
niches_bien <- niches_all %>% filter(period == "modern")
niches_bien_summary <- read.csv("results/niches_summary_bien_worldclim.csv")

niches_all_summary <- ddply(na.omit(niches_all), .(taxon), summarise, 
...
niches_all_summary <- read.csv('results/niches_all_summary.csv')
...
paleoCommClim_means   <- ddply(paleoOcc, .(site, time), summarize, 
                               CommTmax.ave = mean(niches_all_summary$mean.Monthly.maxTemp[niches_all_summary$taxon %in% taxon]),
                               CommTmax.sd  = sd(niches_all_summary$mean.Monthly.maxTemp[niches_all_summary$taxon %in% taxon]),
                               CommPrec.ave = mean(niches_all_summary$mean.Monthly.Prec[niches_all_summary$taxon %in% taxon]),
                               CommPrec.sd  = sd(niches_all_summary$mean.Monthly.Prec[niches_all_summary$taxon %in% taxon]),
                               .progress="text")    
...
source("scripts/PG_Summary Statistics.R")
statistics_sites_Tmax <-ddply(CommClim2, .(site, SiteName, Altitude, Longitude, Latitude),function(x){
  summarizeftct(ft=x$ClimTmax.ave_loess, ct=x$CommTmax.ave_loess, time=x$time, name=x$site)}, .progress = "text") %>% distinct()
...
statistics_sites_Prec <-ddply(CommClim2, .(site, SiteName, Altitude, Longitude, Latitude),function(x){
  summarizeftct(ft=x$ClimPrec.ave_loess, ct=x$CommPrec.ave_loess, time=x$time, name=x$site)}, .progress = "text") %>% distinct()
...
test <- join(CommClim2, statistics_sites_Tmax)
CommClimStat <- join(test, statistics_sites_Prec)
# rm(CommClim2)
save(statistics_sites, file="results/statistics_sites_neotoma.Rdata")



test <- CommClimStat%>% group_by(site) %>% dplyr::summarise(nTime=length(unique(time)))
hist(test$nTime, breaks=45)
min(test$nTime)
CommClimStat <- CommClimStat %>% arrange(site, desc(time))
# CommClimStat <- droplevels(CommClimStat)

save(CommClimStat, file="results/CommClimStat_smoothed.Rdata")
